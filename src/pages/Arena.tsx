...
{pvpReveal && pvpRoundResult?.resolved ? <div className="my-auto grid grid-cols-[1fr_auto_1fr] items-center gap-8 px-8 py-6"><div className="mx-auto w-48">{ARENA_CARDS.find(card => card.id === pvpMyResultCard) && <BattleCard card={ARENA_CARDS.find(card => card.id === pvpMyResultCard)!} compact />}<p className="mt-2 text-center text-xs font-black text-cyan-200">SEU ATAQUE {pvpMyAttack}</p></div><strong className="text-3xl text-white">VS</strong><div className="mx-auto w-48">{ARENA_CARDS.find(card => card.id === pvpOpponentResultCard) && <BattleCard card={ARENA_CARDS.find(card => card.id === pvpOpponentResultCard)!} compact />}<p className="mt-2 text-center text-xs font-black text-purple-200">RIVAL {pvpOpponentAttack}</p></div><div className="col-span-3 text-center"><strong className="text-xl text-amber-200">{pvpRoundResult.winner === 'DRAW' ? 'EMPATE' : `${pvpRoundResult.damage} DE DANO`}</strong></div></div> : <div className="my-auto"><div className="mx-auto grid w-full max-w-[760px] grid-cols-4 items-end gap-3">{deck.map(card => { const used=pvpUsedCards.includes(card.id); const selected=pvpCardId===card.id; return <button key={card.id} type="button" disabled={used || pvpWaitingMove || pvpRoomState.status==='FINISHED'} onClick={() => setPvpCardId(card.id)} className={`relative text-left transition-all ${used?'opacity-25 grayscale':selected?'-translate-y-3 drop-shadow-[0_0_16px_rgba(34,211,238,.45)]':'hover:-translate-y-1'}`}><BattleCard card={card} compact />{used&&<span className="absolute inset-0 flex items-center justify-center rounded-xl bg-black/60 text-[10px] font-black text-slate-200">USADA</span>}</button>})}</div>
