...
export const Arena: React.FC<ArenaProps> = ({ onNavigate }) => {
  ...
  return <div className="mx-auto max-w-5xl space-y-7">
    ...
    <section className="rounded-2xl border border-purple-500/25 bg-[#100d19]/90 p-4 sm:p-6">
      <div className="mb-4 flex items-center justify-between"><div><h2 className="font-heading text-xl font-black text-white">Seu Deck <span className="text-purple-300">{deck.length}/{MAX_DECK_SIZE}</span></h2><p className="mt-1 text-xs text-slate-400">Escolha 4 cartas para começar.</p></div><button type="button" onClick={() => setDeck([])} disabled={!deck.length} className="rounded-lg border border-white/10 px-3 py-2 text-[10px] font-bold uppercase text-slate-300 disabled:opacity-40">Limpar Deck</button></div>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{Array.from({ length: MAX_DECK_SIZE }, (_, index) => deck[index] ? <BattleCard key={deck[index].id} card={deck[index]} compact /> : <div key={index} className="flex min-h-[112px] items-center justify-center rounded-xl border border-dashed border-white/15 text-xs text-slate-600">Slot {index + 1}</div>)}</div>
    </section>
...
const DuelView: React.FC<{ duel: DuelState; setDuel: React.Dispatch<React.SetStateAction<DuelState | null>>; onBack: () => void; onGames: () => void }> = ({ duel, setDuel, onBack, onGames }) => {
  ...
  useEffect(() => {
    ...
    if (duel.phase === 'CHARGE') {
      const timer = window.setTimeout(() => setDuel((current) => {
        ...
        return { ...current, playerHp, cpuHp, playerNexos, cpuNexos, roundDamage: damage, roundMessage: playerWins ? `CPU sofreu -${damage} PV` : cpuWins ? `VOCÊ sofreu -${damage} PV` : 'EMPATE NO CONFRONTO', phase: 'RESULT' };
      }), 1500);
      return () => window.clearTimeout(timer);
    }
    return undefined;
  ...
  return <div className="mx-auto max-w-5xl">
    <div className="relative z-10 flex items-center justify-between px-1 py-1 sm:px-4"><div className="min-w-0"><p className="text-xs font-bold uppercase text-red-300">CPU</p><LifeBar hp={duel.cpuHp} /><NexoBar count={duel.cpuNexos} /></div><p className="px-2 text-center font-heading text-xs font-black uppercase tracking-[0.25em] text-purple-200">Duelo Nexal<br /><span className="font-mono text-purple-400">Rodada {duel.round} / {MAX_ROUNDS}</span></p><div className="min-w-0 text-right"><p className="text-xs font-bold uppercase text-purple-300">JOGADOR</p><div className="flex justify-end"><LifeBar hp={duel.playerHp} /></div><div className="flex justify-end"><NexoBar count={duel.playerNexos} /></div></div></div>
    <div className="relative z-10 mb-1 grid grid-cols-4 gap-2 sm:gap-3">{duel.cpuCards.map((card) => <div key={card.id} className={duel.usedCpu.includes(card.id) ? 'opacity-35 grayscale' : ''}><BattleCard card={card} compact hidden={!duel.revealedCpu || !duel.usedCpu.includes(card.id)} /></div>)}</div>
    <div className="relative overflow-hidden rounded-[28px] border border-purple-400/25 bg-[radial-gradient(circle_at_center,rgba(99,52,160,0.2),transparent_45%),linear-gradient(180deg,#120d20,#080b16)] px-3 py-3 shadow-[0_20px_90px_rgba(35,14,73,0.35)] sm:px-8 sm:py-4">
      ...
      <div className="relative flex min-h-[74px] items-center justify-center gap-3">
        <div className={`absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full border border-purple-300/50 bg-[#180d2d] px-3 py-2 font-heading text-sm font-black text-purple-200 shadow-[0_0_25px_rgba(168,85,247,0.45)] ${duel.phase === 'CHARGE' ? 'scale-110' : ''}`}>⚔ VS ⚔</div>
        {duel.phase !== 'SELECT' && <div className="flex items-center gap-2 animate-[fadeIn_0.4s_ease-out]"><div className="translate-y-1 transition-transform"><BattleCard card={duel.revealedCpu!} compact selected /></div><span className="text-[10px] font-bold text-amber-300">{duel.cpuInvestment} ◆ · {duel.cpuAttack}</span><span className="text-[10px] font-bold text-amber-300">{duel.investment} ◆ · {duel.playerAttack}</span><div className="translate-y-1 transition-transform"><BattleCard card={selectedCard!} compact selected /></div></div>}
        {duel.phase === 'SELECT' && <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">{selectedCard ? 'Carta selecionada — defina seu investimento' : 'Escolha uma carta abaixo'}</p>}
      </div>
      <div className="mt-1 flex justify-center gap-10 text-[10px] font-bold uppercase tracking-widest text-amber-300"><span>{showNumbers ? `${duel.cpuInvestment} NEXOS · ${duel.cpuAttack} ATK` : 'NEXOS OCULTOS'}</span><span>{selectedCard && duel.phase === 'SELECT' ? `${duel.investment} NEXOS · ${attackValue(selectedCard, duel.investment)} ATK` : showNumbers ? `${duel.investment} NEXOS · ${duel.playerAttack} ATK` : 'ESCOLHA UMA CARTA'}</span></div>
      {duel.phase === 'SELECT' && selectedCard && <div className="mx-auto mt-4 max-w-sm rounded-xl border border-cyan-400/30 bg-[#071827]/90 p-3 text-center shadow-[0_0_30px_rgba(34,211,238,0.12)] lg:absolute lg:right-6 lg:top-1/2 lg:mt-0 lg:w-56 lg:-translate-y-1/2"><p className="text-xs font-bold uppercase tracking-widest text-cyan-300">Nexos a investir</p><div className="mt-2 flex items-center justify-center gap-2"><button type="button" onClick={() => setDuel((current) => current ? { ...current, investment: Math.max(0, current.investment - 1) } : current)} className="rounded-lg border border-cyan-300/30 px-3 py-1 text-lg text-white">−</button><NexoBar count={duel.investment} highlighted /><button type="button" disabled={duel.investment >= duel.playerNexos} onClick={() => setDuel((current) => current ? { ...current, investment: Math.min(current.playerNexos, current.investment + 1) } : current)} className="rounded-lg border border-cyan-300/30 px-3 py-1 text-lg text-white disabled:opacity-30">+</button></div><p className="mt-2 text-xs font-bold text-amber-300">ATAQUE PREVISTO: {attackValue(selectedCard, duel.investment)}</p><button type="button" onClick={confirm} className="mt-3 w-full rounded-xl bg-amber-400 px-4 py-3 text-xs font-black uppercase text-slate-950 shadow-[0_0_20px_rgba(251,191,36,0.3)]">Confirmar Jogada</button></div>}
    </div>
    <div className="relative z-10 mt-2"><p className="mb-1 text-center text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400">Seu Deck · escolha uma carta</p><div className="mx-auto grid max-w-2xl grid-cols-4 gap-2">{duel.playerCards.map((card) => { const used = duel.usedPlayer.includes(card.id); return <button key={card.id} type="button" disabled={used || duel.phase !== 'SELECT'} onClick={() => chooseCard(card.id)} className={`text-left transition ${used ? 'opacity-30 grayscale' : ''} ${duel.selectedId === card.id ? '-translate-y-1 ring-2 ring-purple-400' : ''}`}><BattleCard card={card} compact selected={duel.selectedId === card.id} /><span className="block px-2 text-[8px] font-bold uppercase text-slate-500">{used ? '✓ USADA' : 'DISPONÍVEL'}</span></button>; })}</div></div>
  </div>;
};
...
const HiddenCard: React.FC<{ compact?: boolean }> = ({ compact = false }) => <div className={`flex ${compact ? 'min-h-[128px] w-full' : 'h-[220px] w-[165px]'} flex-col items-center justify-center rounded-2xl border-2 border-red-400/30 bg-[radial-gradient(circle,#2b183e,#11121c)] text-center text-red-200 shadow-[0_0_35px_rgba(239,68,68,0.16)]`}><span className={`${compact ? 'text-base' : 'text-2xl'} font-heading font-black`}>NEXA</span><span className={`${compact ? 'my-1 text-xl' : 'my-2 text-3xl'} text-purple-300`}>◇</span><span className="text-[9px] font-bold uppercase tracking-[0.2em]">DUEL<br />Carta oculta</span></div>;
...
const BattleCard: React.FC<{ card: ArenaCard; compact?: boolean; battle?: boolean; selected?: boolean; hidden?: boolean }> = ({ card, compact = false, battle = false, selected = false, hidden = false }) => hidden ? <HiddenCard compact /> : <div className={`relative rounded-2xl border ${selected ? 'border-cyan-300 shadow-[0_0_32px_rgba(34,211,238,0.38)]' : card.rarity === 'Lendário' ? 'border-amber-400/60 shadow-[0_0_20px_rgba(251,191,36,0.12)]' : 'border-white/15'} bg-gradient-to-b from-[#1d1b31] to-[#11121c] ${battle ? 'h-[220px] w-[165px] p-3 sm:h-[240px] sm:w-[180px] sm:p-3' : compact ? 'min-h-[128px] p-2' : 'min-h-[145px] p-3'}`}><div className="text-center text-[8px] font-bold uppercase tracking-widest text-purple-300">{card.rarity}</div><p className={`mt-0.5 text-center font-heading font-black text-white ${battle ? 'text-base' : 'text-xs'}`}>{card.name}</p><div className={`flex items-center justify-center ${battle ? 'h-24 text-5xl' : 'h-5 text-base'}`}>{ELEMENT_ICONS[card.element] || '✦'}</div>{!compact && !battle && <p className="mt-2 text-xs text-slate-400">{card.description}</p>}<div className="absolute bottom-2 left-2 right-2 flex justify-between text-[10px] font-mono text-slate-200"><span><b className="text-red-300">POD</b> {card.power}</span><span><b className="text-pink-300">DANO</b> {card.damage}</span></div>{battle && card.ability && <p className="absolute bottom-7 left-2 right-2 text-center text-[9px] text-amber-200">{card.ability}</p>}{selected && <Check className="absolute right-2 top-2 h-3 w-3 text-cyan-300" />}</div>;
