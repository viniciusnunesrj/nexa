...
const DuelView: React.FC<{ duel: DuelState; setDuel: React.Dispatch<React.SetStateAction<DuelState | null>>; onBack: () => void; onGames: () => void }> = ({ duel, setDuel, onBack, onGames }) => {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="relative z-10 mb-1 flex justify-center gap-1.5"><span className="mr-1 self-center text-[8px] font-bold uppercase tracking-widest text-slate-500">CPU</span>{duel.cpuCards.map((card) => <div key={card.id} className={`flex h-7 w-12 items-center justify-center rounded border bg-[#171323] text-[9px] ${duel.usedCpu.includes(card.id) ? 'border-red-400/50 text-red-300 opacity-50' : 'border-purple-300/30 text-purple-200/70'}`}>{duel.usedCpu.includes(card.id) ? '✓ USADA' : '◇ NEXA'}</div>)}</div>
    <div className="relative overflow-hidden rounded-[28px] border border-purple-400/25 bg-[radial-gradient(circle_at_center,rgba(99,52,160,0.2),transparent_45%),linear-gradient(180deg,#120d20,#080b16)] px-3 py-3 shadow-[0_20px_90px_rgba(35,14,73,0.35)] sm:px-8 sm:py-4">
      ...
      <div className="relative flex items-center justify-center gap-4 sm:gap-8">
        <div className={duel.revealedCpu ? 'animate-[fadeIn_0.7s_ease-out]' : ''}>{duel.revealedCpu ? <BattleCard card={duel.revealedCpu} battle /> : <HiddenCard />}</div>
        ...
        <div className={duel.phase === 'RESULT' ? 'scale-105 transition-transform' : duel.phase === 'CHARGE' ? 'translate-y-1 transition-transform' : ''}>{selectedCard ? <BattleCard card={selectedCard} battle selected={duel.phase === 'SELECT'} /> : <div className="flex h-[220px] w-[165px] items-center justify-center rounded-2xl border border-dashed border-purple-400/40 text-center text-xs text-slate-500">ESCOLHA UMA<br />CARTA</div>}</div>
      </div>
...
const HiddenCard: React.FC = () => <div className="flex h-[220px] w-[165px] flex-col items-center justify-center rounded-2xl border-2 border-red-400/30 bg-[radial-gradient(circle,#2b183e,#11121c)] text-center text-red-200 shadow-[0_0_35px_rgba(239,68,68,0.16)]"><span className="font-heading text-2xl font-black">NEXA</span><span className="my-2 text-3xl text-purple-300">◇</span><span className="text-[10px] font-bold uppercase tracking-[0.25em]">DUEL<br />Carta oculta</span></div>;
