...
const DuelView: React.FC<{ duel: DuelState; setDuel: React.Dispatch<React.SetStateAction<DuelState | null>>; onBack: () => void; onGames: () => void }> = ({ duel, setDuel, onBack, onGames }) => {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="relative z-10 mb-2 flex justify-center gap-1.5"><span className="mr-2 self-center text-[9px] font-bold uppercase tracking-widest text-slate-500">Reserva CPU</span>{duel.cpuCards.map((card) => <div key={card.id} className={`flex h-7 w-6 items-center justify-center rounded border bg-[#171323] text-[10px] ${duel.usedCpu.includes(card.id) ? 'border-red-400/50 text-red-300' : 'border-purple-300/30 text-purple-200/70'}`}>{duel.usedCpu.includes(card.id) ? '✓' : '◇'}</div>)}</div>
...
const BattleCard: React.FC<{ card: ArenaCard; compact?: boolean; battle?: boolean; selected?: boolean }> = ({ card, compact = false, battle = false, selected = false }) => <div className={`relative rounded-2xl border ${selected ? 'border-cyan-300 shadow-[0_0_32px_rgba(34,211,238,0.38)]' : card.rarity === 'Lendário' ? 'border-amber-400/60 shadow-[0_0_20px_rgba(251,191,36,0.12)]' : 'border-white/15'} bg-gradient-to-b from-[#1d1b31] to-[#11121c] ${battle ? 'h-[320px] w-[240px] p-4 sm:h-[350px] sm:w-[260px] sm:p-5' : compact ? 'min-h-[105px] p-3' : 'min-h-[145px] p-3'}`}><div className="text-center text-[9px] font-bold uppercase tracking-widest text-purple-300">{card.rarity}</div><p className={`mt-1 text-center font-heading font-black text-white ${battle ? 'text-lg' : 'text-sm'}`}>{card.name}</p><div className={`flex items-center justify-center ${battle ? 'h-40 text-7xl' : 'h-8 text-xl'}`}>{ELEMENT_ICONS[card.element] || '✦'}</div>{!compact && !battle && <p className="mt-2 text-xs text-slate-400">{card.description}</p>}<div className="absolute bottom-3 left-3 right-3 flex justify-between text-xs font-mono text-slate-200"><span><b className="text-red-300">POD</b> {card.power}</span><span><b className="text-pink-300">DANO</b> {card.damage}</span></div>{battle && card.ability && <p className="absolute bottom-10 left-3 right-3 text-center text-[10px] text-amber-200">{card.ability}</p>}{selected && <Check className="absolute right-3 top-3 h-4 w-4 text-cyan-300" />}</div>;
