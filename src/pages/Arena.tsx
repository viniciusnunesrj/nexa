...
export const Arena: React.FC<ArenaProps> = ({ onNavigate }) => {
  ...
  const [pvpBattleOpen, setPvpBattleOpen] = useState(false);
  const [pvpCardId, setPvpCardId] = useState<string | null>(null);
  const [pvpNexos, setPvpNexos] = useState(0);
  const [pvpRoundResult, setPvpRoundResult] = useState<any>(null);
  const [pvpSubmitting, setPvpSubmitting] = useState(false);
  ...
  const playPvpReveal = (result: any, resolvedRound: number, resetChoice = true) => {
    ...
    pvpRevealTimers.current = [];
    setPvpRoundResult(result);
  ...
  const createPvpRoom = async () => { if (!canStart) return; setPvpUsedCards([]); setPvpOpponentUsedCount(0); setPvpLastResolvedRound(0); setPvpRoundResult(null); setPvpHistory([]); setPvpWaitingMove(false); setPvpReveal(false); setPvpCardId(null); setPvpNexos(0); setPvpStatus('Criando sala...'); const { data, error } = await supabase.rpc('create_duelo_nexal_pvp_room', { p_deck: deck.map(card => card.id) }); if (error || !data) { setPvpStatus('Não foi possível criar a sala.'); return; } const room = Array.isArray(data) ? data[0] : data; setPvpCode(room.code); setPvpRoomId(room.id); setPvpRoomState(room); setPvpStatus('Sala criada. Compartilhe o código e aguarde o adversário.'); };
  const joinPvpRoom = async () => { if (!canStart || !pvpCode.trim()) return; setPvpUsedCards([]); setPvpOpponentUsedCount(0); setPvpLastResolvedRound(0); setPvpRoundResult(null); setPvpHistory([]); setPvpWaitingMove(false); setPvpReveal(false); setPvpCardId(null); setPvpNexos(0); setPvpStatus('Entrando na sala...'); const { data, error } = await supabase.rpc('join_duelo_nexal_pvp_room', { p_code: pvpCode.trim().toUpperCase(), p_deck: deck.map(card => card.id) }); if (error || !data) { setPvpStatus('Sala indisponível ou código inválido.'); return; } const room = Array.isArray(data) ? data[0] : data; setPvpRoomId(room.id); setPvpRoomState(room); setPvpStatus('Adversário conectado. Sala pronta.'); };
  const pvpIsHost = !!currentUser && pvpRoomState?.hostId === currentUser.id;
  const pvpDeckKey = deck.map((card) => card.id).join('|');
  const pvpMyDeckIds = useMemo(() => new Set(deck.map((card) => card.id)), [pvpDeckKey]);
  useEffect(() => {
    ...
    if (resolvedCount <= 0 || pvpLastResolvedRound >= resolvedCount) return;
    let cancelled = false;
    const restoreHistory = async () => {
      setPvpResumeSyncing(true);
      const usedMine: string[] = [];
      for (let round = 1; round <= resolvedCount; round += 1) {
      ...
      if (!cancelled) {
        ...
        setPvpOpponentUsedCount(resolvedCount);
        setPvpLastResolvedRound(resolvedCount);
        setPvpResumeSyncing(false);
      }
    };
    restoreHistory();
  ...
  useEffect(() => {
    ...
    if (data.status === 'READY' || data.status === 'PLAYING') { if ((pvpWaitingMove || serverSubmitted) && Number(data.round) > pvpLastResolvedRound) { const resolvedRound = Number(data.round) - 1; const { data: result } = await supabase.rpc('get_duelo_nexal_pvp_round_result', { p_room_id: pvpRoomId, p_round: resolvedRound }); if (result?.resolved) { setPvpOpponentUsedCount((count) => Math.max(count, resolvedRound)); playPvpReveal(result, resolvedRound); } setPvpWaitingMove(false); setPvpStatus('Rodada resolvida.'); } else if (!pvpWaitingMove) setPvpStatus('Adversário conectado. Sala pronta para o duelo.'); } if (data.status === 'FINISHED') { if (pvpWaitingMove) { const finalRound = Number(data.round); const { data: result } = await supabase.rpc('get_duelo_nexal_pvp_round_result', { p_room_id: pvpRoomId, p_round: finalRound }); if (result?.resolved && finalRound > pvpLastResolvedRound) { playPvpReveal(result, finalRound, false); } } setPvpWaitingMove(false); setPvpStatus('Partida finalizada.'); } };
  ...
  const pvpOpponentHp = pvpIsHost ? pvpRoomState?.guestHp : pvpRoomState?.hostHp;
  const pvpMyNexos = pvpIsHost ? pvpRoomState?.hostNexos : pvpRoomState?.guestNexos;
  const pvpOpponentNexos = pvpIsHost ? pvpRoomState?.guestNexos : pvpRoomState?.hostNexos;
  const pvpMyResultCard = pvpIsHost ? pvpRoundResult?.hostCard : pvpRoundResult?.guestCard;
  ...
  const forfeitPvp = async () => {
    if (!pvpRoomId || pvpRoomState?.status === 'FINISHED') return;
    if (!window.confirm('Desistir desta partida PvP? O adversário receberá a vitória.')) return;
    setPvpStatus('Encerrando partida...');
    const { data, error } = await supabase.rpc('forfeit_duelo_nexal_pvp_room', { p_room_id: pvpRoomId });
    if (error || !data) { setPvpStatus('Não foi possível encerrar a partida.'); return; }
    setPvpRoomState((previous: any) => ({ ...previous, ...data }));
    setPvpWaitingMove(false);
    setPvpReveal(false);
    setPvpStatus('Você desistiu da partida.');
  };
  const submitPvpMove = async () => {
    if (!pvpRoomId || !pvpCardId || pvpSubmitting) return;
    setPvpSubmitting(true);
    const submittedCardId = pvpCardId; setPvpRoundResult(null); setPvpStatus('Jogada enviada. Aguardando adversário...');
    const { data, error } = await supabase.rpc('submit_duelo_nexal_pvp_move', { p_room_id: pvpRoomId, p_card_id: pvpCardId, p_nexos: pvpNexos });
    setPvpSubmitting(false);
    if (error) { setPvpStatus('Não foi possível enviar a jogada.'); return; }
    setPvpUsedCards((used) => submittedCardId && !used.includes(submittedCardId) ? [...used, submittedCardId] : used); if (data?.resolved) setPvpOpponentUsedCount((count) => Math.max(count, Number(data.round) || count + 1)); setPvpRoundResult(data); setPvpWaitingMove(!data?.resolved); if (data?.resolved) { playPvpReveal(data, Number(pvpRoomState?.round || 1)); } setPvpStatus(data?.resolved ? 'Rodada resolvida.' : 'Jogada confirmada. Aguardando adversário...');
  };
  if (duel) return <DuelView duel={duel} setDuel={setDuel} onBack={() => setDuel(null)} onGames={() => onNavigate('games')} />;
  return <div className="arena-lobby mx-auto max-w-5xl space-y-5"><header className="relative overflow-hidden rounded-2xl border border-cyan-400/25 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,.18),transparent_45%),#0a0e1c] p-6 sm:p-8"><div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-mono font-bold tracking-[.3em] text-cyan-300">NEXA / ARENA</p><h1 className="mt-1 font-heading text-3xl font-black text-white sm:text-4xl">NEXA ARENA</h1><p className="mt-2 max-w-xl text-sm text-slate-300">Monte seu esquadrão e entre no Duelo Nexal.</p><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 text-[9px] font-black tracking-[.14em] text-cyan-200">PVE · DISPONÍVEL</span><span className="rounded-full border border-purple-400/30 bg-purple-400/10 px-3 py-1 text-[9px] font-black tracking-[.14em] text-purple-200">PVP · EM PREPARAÇÃO</span></div></div><button type="button" onClick={() => onNavigate('games')} className="inline-flex w-fit items-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-xs font-black uppercase text-slate-200"><ArrowLeft className="h-4 w-4" /> Voltar para Jogos</button></div></header><section className="arena-deck-panel rounded-2xl border border-cyan-400/20 bg-[#0b1020]/90 p-4 sm:p-6"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-heading text-xl font-black text-white">Seu Deck <span className="text-cyan-300">{deck.length}/{MAX_DECK_SIZE}</span></h2><p className="mt-1 text-xs text-slate-400">Escolha 4 cartas para começar.</p></div><button type="button" onClick={() => setDeck([])} disabled={!deck.length} className="rounded-lg border border-white/10 px-3 py-2 text-[10px] font-bold uppercase text-slate-300 disabled:opacity-40">Limpar Deck</button></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{Array.from({ length: MAX_DECK_SIZE }, (_, index) => deck[index] ? <BattleCard key={deck[index].id} card={deck[index]} compact /> : <div key={index} className="flex min-h-[112px] items-center justify-center rounded-xl border border-dashed border-white/15 text-xs text-slate-600">Slot {index + 1}</div>)}</div></section><section className="rounded-2xl border border-white/10 bg-[#090d18]/70 p-4 sm:p-5"><div className="mb-4 flex flex-col gap-3"><div className="flex items-end justify-between"><div><h2 className="font-heading text-xl font-black text-white">Cartas para o Duelo</h2><p className="mt-1 text-xs text-slate-400">PODER, DANO e habilidade são o foco da Arena.</p></div><span className="text-xs font-mono text-slate-500">{visibleCards.length}/{ARENA_CARDS.length}</span></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setCollectionView('AVAILABLE')} className={`rounded-lg border px-3 py-2 text-[10px] font-black uppercase ${collectionView === 'AVAILABLE' ? 'border-cyan-300 bg-cyan-400/10 text-cyan-200' : 'border-white/10 text-slate-400'}`}>Disponíveis</button><button type="button" onClick={() => setCollectionView('ALL')} className={`rounded-lg border px-3 py-2 text-[10px] font-black uppercase ${collectionView === 'ALL' ? 'border-cyan-300 bg-cyan-400/10 text-cyan-200' : 'border-white/10 text-slate-400'}`}>Todas</button>{['Todas','Comum','Incomum','Raro','Épico','Lendário','Mítico'].map((rarity) => <button key={rarity} type="button" onClick={() => setRarityFilter(rarity)} className={`rounded-lg border px-2.5 py-2 text-[9px] font-bold ${rarityFilter === rarity ? 'border-purple-300 bg-purple-400/10 text-purple-200' : 'border-white/10 text-slate-500'}`}>{rarity}</button>)}</div></div>{visibleCards.length ? <div className="arena-card-grid grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{visibleCards.map((card) => <button key={card.id} type="button" disabled={!isAvailable(card)} onClick={() => toggleCard(card)} className={`arena-card-choice text-left disabled:opacity-40 disabled:cursor-not-allowed ${deck.some((item) => item.id === card.id) ? 'arena-card-selected ring-2 ring-cyan-300' : ''}`}><BattleCard card={card} selected={deck.some((item) => item.id === card.id)} compact /><span className="mt-1 block text-center text-[9px] font-bold tracking-wider text-cyan-200">{card.starter ? 'STARTER' : isAvailable(card) ? 'DISPONÍVEL' : 'NÃO POSSUÍDA'}</span></button>)}</div> : <div className="rounded-xl border border-dashed border-white/10 p-8 text-center text-xs text-slate-500">Nenhuma carta neste filtro.</div>}</section><div className="grid gap-3 sm:grid-cols-2"><button type="button" disabled={!canStart} onClick={() => { if (canStart) setDuel(freshDuel(deck)); }} className="arena-start flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 px-5 py-4 font-heading text-sm font-black uppercase tracking-[.14em] text-white disabled:cursor-not-allowed disabled:opacity-40"><Swords className="h-5 w-5" /> Duelo PVE</button><button type="button" disabled={!canStart} onClick={() => setPvpOpen((value) => !value)} className="flex w-full items-center justify-center gap-2 rounded-xl border border-purple-400/35 bg-purple-500/10 px-5 py-4 font-heading text-sm font-black uppercase tracking-[.14em] text-purple-200 disabled:opacity-40"><Swords className="h-5 w-5" /> PVP · Sala privada</button></div>{pvpOpen && <div className="mt-3 rounded-xl border border-purple-400/20 bg-[#120d24] p-4"><p className="text-xs text-slate-300">Use seu deck atual de 4 cartas. Crie uma sala ou digite o código recebido.</p><div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto_auto]"><input value={pvpCode} onChange={(e) => setPvpCode(e.target.value.toUpperCase().slice(0,6))} placeholder="CÓDIGO DA SALA" className="rounded-lg border border-white/10 bg-black/30 px-3 py-3 font-mono text-sm font-black tracking-[.2em] text-white outline-none focus:border-purple-300" /><button type="button" onClick={createPvpRoom} className="rounded-lg bg-purple-600 px-4 py-3 text-xs font-black uppercase text-white">Criar sala</button><button type="button" disabled={!pvpCode.trim()} onClick={joinPvpRoom} className="rounded-lg border border-purple-300/40 px-4 py-3 text-xs font-black uppercase text-purple-100 disabled:opacity-40">Entrar</button></div>{pvpStatus && <p className="mt-3 text-xs font-bold text-purple-200">{pvpStatus}{pvpCode && pvpStatus.startsWith('Sala criada') ? ` · CÓDIGO: ${pvpCode}` : ''}</p>}{pvpRoomState && <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-black uppercase"><span className="rounded-full border border-white/10 px-3 py-1 text-slate-300">Sala {pvpRoomState.code || pvpCode}</span><span className="rounded-full border border-purple-400/30 px-3 py-1 text-purple-200">{pvpRoomState.guestId || pvpRoomState.guest_id ? '2/2 jogadores' : '1/2 jogadores'}</span><span className="rounded-full border border-cyan-400/30 px-3 py-1 text-cyan-200">{pvpRoomState.status}</span></div>}{pvpRoomState?.status === 'WAITING' && <button type="button" onClick={cancelWaitingPvp} className="mt-3 w-full rounded-lg border border-rose-400/20 px-4 py-3 text-xs font-black uppercase text-rose-300">Cancelar sala</button>}{pvpRoomState && (pvpRoomState.guestId || pvpRoomState.guest_id) && <button type="button" onClick={() => setPvpBattleOpen(true)} className="mt-3 w-full rounded-lg bg-gradient-to-r from-purple-600 to-cyan-600 px-4 py-3 text-xs font-black uppercase text-white">Entrar no tabuleiro PvP</button>}{pvpBattleOpen && pvpRoomState && <div className="pvp-board fixed inset-0 z-[9999] overflow-hidden bg-[#030711] p-2 sm:p-3"><div className="mx-auto flex h-[calc(100dvh-1rem)] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-cyan-400/20 bg-[radial-gradient(circle_at_center,#10213b_0%,#07101f_42%,#030711_78%)] p-2 shadow-2xl sm:h-[calc(100dvh-1.5rem)] sm:p-3"><div className="flex items-center justify-between border-b border-white/10 pb-3"><div><p className="text-[9px] font-black tracking-[.22em] text-purple-300">DUELO NEXAL · PVP</p><strong className="text-sm text-white">SALA {pvpRoomState.code || pvpCode}</strong></div><div className="flex gap-2"><button type="button" onClick={() => setPvpBattleOpen(false)} className="rounded-lg border border-white/10 px-3 py-2 text-[9px] font-black uppercase text-slate-300">Voltar ao lobby</button>{pvpRoomState.status !== 'FINISHED' && <button type="button" onClick={forfeitPvp} className="rounded-lg border border-rose-400/20 px-3 py-2 text-[9px] font-black uppercase text-rose-300">Desistir</button>}</div></div><div className="mt-2 flex shrink-0 items-center justify-between rounded-xl border border-purple-400/15 bg-purple-500/5 px-3 py-2"><div><span className="block text-[8px] font-black uppercase tracking-[.18em] text-purple-300">Adversário</span><strong className="text-lg text-white">PV {pvpOpponentHp ?? 12}</strong></div><div className="pvp-opponent-hand flex gap-1">{[0,1,2,3].map(i => { const used = i < pvpOpponentUsedCount; return <div key={i} className={`pvp-card-back relative h-12 w-9 rounded-md border border-purple-300/20 bg-[linear-gradient(135deg,#28174b,#10172b_55%,#06101c)] shadow-[0_0_14px_#a855f722] transition-all ${used ? 'pvp-opponent-used opacity-25' : ''}`}><span className="absolute inset-1 rounded border border-purple-300/10" /><span className="absolute inset-0 flex items-center justify-center text-[7px] font-black tracking-[.12em] text-purple-200/50">NEXA</span>{used && <span className="absolute inset-0 flex items-center justify-center text-[7px] font-black text-slate-400">USADA</span>}</div> })}</div><strong className="text-xs text-purple-200">{pvpOpponentNexos ?? 12} NEXOS</strong></div><div className="relative my-2 flex min-h-0 flex-1 flex-col justify-between overflow-hidden rounded-2xl border border-white/5 bg-black/20 p-2 sm:p-3">{pvpRoundTransition && pvpRoomState.status !== 'FINISHED' && <div className="pvp-round-transition absolute inset-0 z-40 flex items-center justify-center rounded-2xl bg-[#030711]/90 backdrop-blur-sm"><div className="text-center"><span className="block text-[8px] font-black tracking-[.25em] text-cyan-300">DUELO NEXAL</span><strong className="mt-1 block text-3xl font-black text-white">RODADA {pvpRoomState.round || 1}</strong><span className="mt-2 block text-[9px] font-bold uppercase tracking-[.14em] text-slate-400">Prepare sua próxima carta</span></div></div>}<div className="flex items-center justify-between"><div className="flex items-center gap-2"><strong className="text-sm text-white">RODADA {pvpRoomState.round || 1}</strong>{pvpResumeSyncing && <span className="text-[8px] font-black tracking-[.12em] text-purple-300">SINCRONIZANDO...</span>}{pvpWaitingMove && <span className="rounded-full border border-cyan-400/20 bg-cyan-400/5 px-2 py-1 text-[7px] font-black tracking-[.1em] text-cyan-200">VOCÊ · PRONTO</span>}</div><span className="text-[10px] font-black text-cyan-200">PV {pvpMyHp ?? 12} × {pvpOpponentHp ?? 12}</span></div><p className="mt-2 text-xs text-slate-400">Escolha sua carta e invista Nexos. A escolha fica secreta até o adversário jogar.</p><div className="pvp-player-hand mx-auto mt-2 grid w-full max-w-[680px] grid-cols-4 items-end gap-1.5 sm:gap-2 [&_.arena-battle-card]:max-h-[30vh]">{deck.map(card => { const used = pvpUsedCards.includes(card.id); const selected = pvpCardId === card.id; return <button key={card.id} type="button" disabled={used || pvpWaitingMove || pvpRoomState.status === 'FINISHED'} onClick={() => setPvpCardId(card.id)} className={`pvp-hand-card relative text-left transition-all duration-300 ${used ? 'pvp-hand-used opacity-35 grayscale' : selected ? 'pvp-hand-selected -translate-y-3 drop-shadow-[0_0_16px_rgba(34,211,238,.45)]' : 'hover:-translate-y-1'}`}><BattleCard card={card} compact selected={selected} />{used && <span className="absolute inset-0 z-20 flex items-center justify-center rounded-xl bg-black/55 text-[9px] font-black tracking-[.18em] text-slate-200">USADA</span>}{selected && !used && <span className="absolute -bottom-2 left-1/2 z-20 -translate-x-1/2 rounded-full border border-cyan-300/40 bg-[#06131d] px-2 py-1 text-[7px] font-black tracking-[.12em] text-cyan-200">SELECIONADA</span>}</button> })}</div><div className="mx-auto mt-2 flex w-full max-w-[680px] shrink-0 flex-wrap items-center justify-center gap-2 rounded-xl border border-cyan-400/10 bg-[#06101d]/80 p-3"><input type="range" min="0" max={Math.min(12, pvpMyNexos ?? 12)} value={pvpNexos} onChange={(e) => setPvpNexos(Number(e.target.value))} className="min-w-[150px] flex-1" /><strong className="text-xs text-purple-200">{pvpNexos} NEXOS</strong><button type="button" disabled={!pvpCardId || pvpSubmitting || pvpWaitingMove || pvpResumeSyncing || pvpRoomState.status === 'FINISHED'} onClick={submitPvpMove} className="rounded-lg bg-cyan-500 px-4 py-2 text-[10px] font-black uppercase text-[#04131b] disabled:opacity-40">{pvpSubmitting ? 'Enviando...' : 'Confirmar'}</button></div>{pvpReveal && pvpRoundResult?.resolved && <div className={`pvp-reveal-stage pvp-step-${pvpRevealStep} absolute inset-2 z-30 overflow-hidden rounded-xl border border-cyan-400/20 bg-[#050a13] p-4`}><p className="text-center text-[9px] font-black tracking-[.22em] text-cyan-300">CONFRONTO · RODADA RESOLVIDA</p><div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><div className="pvp-reveal-card pvp-host">{ARENA_CARDS.find(card => card.id === pvpMyResultCard) && <BattleCard card={ARENA_CARDS.find(card => card.id === pvpMyResultCard)!} compact selected={pvpIWonRound} />}<div className="pvp-combat-stat"><span>SEU ATAQUE</span><b>{pvpMyAttack}</b><small>{pvpMySpent} NEXOS</small></div></div><strong className="pvp-vs text-xl text-white">VS</strong><div className="pvp-reveal-card pvp-guest">{ARENA_CARDS.find(card => card.id === pvpOpponentResultCard) && <BattleCard card={ARENA_CARDS.find(card => card.id === pvpOpponentResultCard)!} compact selected={pvpOpponentWonRound} />}<div className="pvp-combat-stat"><span>ATAQUE RIVAL</span><b>{pvpOpponentAttack}</b><small>{pvpOpponentSpent} NEXOS</small></div></div></div><div className="pvp-impact mt-4 text-center"><strong className="text-lg text-amber-200">{pvpRoundResult.winner === 'DRAW' ? 'EMPATE' : `${pvpRoundResult.damage} DE DANO`}</strong><p className="mt-1 text-[10px] font-bold text-slate-400">PV · {pvpIsHost ? pvpRoundResult.hostHp : pvpRoundResult.guestHp} × {pvpIsHost ? pvpRoundResult.guestHp : pvpRoundResult.hostHp}</p></div></div>}{pvpWaitingMove && <div className="mt-3 rounded-lg border border-purple-400/20 bg-purple-500/5 p-3 text-center"><span className="pvp-wait-dot inline-block h-2 w-2 rounded-full bg-purple-300" /><p className="mt-2 text-[10px] font-black uppercase tracking-[.15em] text-purple-200">Sua jogada está bloqueada</p><p className="mt-1 text-[9px] text-slate-400">Aguardando a escolha secreta do adversário...</p></div>}{pvpHistory.length > 0 && <div className="mt-3 flex flex-wrap justify-center gap-1.5">{pvpHistory.map((item) => { const mine = pvpIsHost ? item.hostAttack : item.guestAttack; const rival = pvpIsHost ? item.guestAttack : item.hostAttack; const won = item.winner === (pvpIsHost ? 'HOST' : 'GUEST'); const lost = item.winner === (pvpIsHost ? 'GUEST' : 'HOST'); return <span key={item.round} className={`rounded-full border px-2 py-1 text-[8px] font-black ${won ? 'border-cyan-400/25 text-cyan-200' : lost ? 'border-rose-400/20 text-rose-300' : 'border-white/10 text-slate-400'}`}>R{item.round} · {mine}×{rival}</span> })}</div>}{pvpRoundResult?.resolved && <div className="mt-3 rounded-lg border border-white/10 p-3 text-xs text-slate-200">Ataque: {pvpMyAttack} × {pvpOpponentAttack} · Dano {pvpRoundResult.damage} · {pvpRoundResult.status === 'FINISHED' ? 'PARTIDA ENCERRADA' : 'PRÓXIMA RODADA'}</div>}{pvpRoomState?.status === 'FINISHED' && !pvpReveal && <div className={`pvp-finish relative mt-4 overflow-hidden rounded-2xl border p-5 text-center ${!pvpRoomState.winnerId ? 'border-slate-300/20 bg-slate-400/5' : pvpRoomState.winnerId === currentUser?.id ? 'border-cyan-300/30 bg-cyan-400/5' : 'border-rose-300/25 bg-rose-400/5'}`}><div className="pvp-finish-ring absolute left-1/2 top-1/2 h-40 w-40 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10" /><p className="relative text-[9px] font-black tracking-[.24em] text-amber-200">FIM DO DUELO PVP</p><strong className="pvp-finish-title relative mt-2 block text-3xl font-black text-white">{!pvpRoomState.winnerId ? 'EMPATE' : pvpRoomState.winnerId === currentUser?.id ? 'VITÓRIA' : 'DERROTA'}</strong><p className="relative mt-2 text-xs text-slate-300">PV FINAL · VOCÊ {pvpMyHp} × {pvpOpponentHp} RIVAL</p><p className="relative mt-1 text-[9px] font-bold uppercase tracking-[.12em] text-slate-500">Partida concluída no servidor</p><div className="relative mt-4 flex justify-center gap-2"><button type="button" onClick={() => { setPvpBattleOpen(false); setPvpRoomState(null); setPvpRoomId(null); setPvpCode(''); setPvpStatus(''); setPvpUsedCards([]); setPvpOpponentUsedCount(0); setPvpLastResolvedRound(0); setPvpRoundResult(null); setPvpCardId(null); setPvpNexos(0); }} className="rounded-lg border border-white/10 px-4 py-2 text-[9px] font-black uppercase text-slate-200">Sair da sala</button></div></div>}</div><div className="mt-4 rounded-xl border border-cyan-400/15 bg-cyan-500/5 p-3"><div className="mb-3 flex items-center justify-between"><div><span className="block text-[8px] font-black uppercase tracking-[.18em] text-cyan-300">Você</span><strong className="text-lg text-white">PV {pvpMyHp ?? 12}</strong></div><strong className="text-xs text-cyan-200">{pvpMyNexos ?? 12} NEXOS</strong></div></div></div></div>}</div>}<style>{`
  ...
  .pvp-nexos-range{accent-color:#22d3ee}.pvp-nexos-range:disabled{opacity:.35}.pvp-round-transition{animation:pvpRoundTransition .85s ease both}@keyframes pvpRoundTransition{0%{opacity:0}20%,75%{opacity:1}100%{opacity:0}}.pvp-round-transition strong{animation:pvpRoundNumber .65s cubic-bezier(.2,.9,.2,1) both}@keyframes pvpRoundNumber{from{opacity:0;transform:scale(.72);letter-spacing:.3em}to{opacity:1;transform:scale(1);letter-spacing:.02em}}.pvp-finish{animation:pvpFinishIn .55s cubic-bezier(.2,.8,.2,1) both}.pvp-finish-ring{animation:pvpFinishRing 1.8s ease-out infinite}.pvp-finish-title{animation:pvpFinishTitle .7s cubic-bezier(.2,.9,.2,1) both}@keyframes pvpFinishIn{from{opacity:0;transform:scale(.94)}to{opacity:1;transform:scale(1)}}@keyframes pvpFinishRing{0%{opacity:.45;transform:translate(-50%,-50%) scale(.55)}100%{opacity:0;transform:translate(-50%,-50%) scale(1.35)}}@keyframes pvpFinishTitle{from{opacity:0;transform:translateY(10px) scale(.9)}to{opacity:1;transform:none}}.pvp-opponent-hand{perspective:700px}.pvp-card-back{transform:rotateX(-4deg);transform-origin:top center}.pvp-opponent-used{transform:translateY(-.25rem) scale(.92)}.pvp-player-hand{perspective:900px}.pvp-hand-card{min-width:0;transform-origin:bottom center}.pvp-hand-card:disabled{cursor:not-allowed}.pvp-hand-selected{z-index:10}.pvp-hand-used{transform:translateY(.35rem) scale(.94)}@media(max-width:640px){.pvp-player-hand{gap:.35rem}.pvp-hand-card{font-size:.8rem}.pvp-hand-card .battle-card{min-width:0}}.pvp-reveal-stage{position:relative;animation:pvp-stage-in .35s ease-out both}.pvp-reveal-card{position:relative;max-width:11rem;margin:auto;padding:.45rem;border:1px solid #ffffff18;border-radius:.7rem;background:#ffffff08;animation:pvp-card-enter .55s cubic-bezier(.2,.8,.2,1) both}.pvp-host{transform-origin:right center}.pvp-guest{transform-origin:left center;animation-name:pvp-card-enter-right}.pvp-reveal-stage:after{content:"";position:absolute;inset:0;pointer-events:none;opacity:0;background:radial-gradient(circle at center,#67e8f933,transparent 55%)}.pvp-step-3:after{animation:pvp-flash .55s ease-out}.pvp-step-1 .pvp-combat-stat{opacity:.2}.pvp-step-1 .pvp-impact,.pvp-step-2 .pvp-impact{opacity:0}.pvp-step-2 .pvp-combat-stat{opacity:1}.pvp-step-3 .pvp-reveal-card{filter:brightness(1.16);transform:scale(1.025)}.pvp-step-4 .pvp-reveal-card:not(:has(.ring-cyan-300)){opacity:.62;filter:saturate(.65)}.pvp-combat-stat{margin-top:.35rem;display:grid;grid-template-columns:1fr auto;align-items:center;gap:.1rem .4rem;padding:.35rem .45rem;border-radius:.5rem;background:#020617cc;border:1px solid #ffffff12}.pvp-combat-stat span{font-size:.48rem;font-weight:900;letter-spacing:.12em;color:#94a3b8}.pvp-combat-stat b{grid-row:1/3;grid-column:2;font-size:1.25rem;color:#fde68a;text-shadow:0 0 .7rem #f59e0b88;animation:pvp-attack-charge .8s .55s ease-out both}.pvp-combat-stat small{font-size:.48rem;color:#67e8f9}.pvp-reveal-card:has(.ring-cyan-300){animation-duration:.55s;box-shadow:0 0 1.5rem #22d3ee55}.pvp-vs{animation:pvp-vs-pop .7s .35s ease-out both}.pvp-impact{animation:pvp-impact-in .45s 1.05s ease-out both}@keyframes pvp-stage-in{from{opacity:0}to{opacity:1}}@keyframes pvp-card-enter{from{opacity:0;transform:translateX(-45%) scale(.8)}to{opacity:1;transform:none}}@keyframes pvp-card-enter-right{from{opacity:0;transform:translateX(45%) scale(.8)}to{opacity:1;transform:none}}@keyframes pvp-vs-pop{0%{opacity:0;transform:scale(.2)}65%{opacity:1;transform:scale(1.3)}100%{transform:scale(1)}}@keyframes pvp-flash{0%{opacity:0}35%{opacity:1}100%{opacity:0}}@keyframes pvp-attack-charge{0%{opacity:.25;transform:scale(.6)}65%{opacity:1;transform:scale(1.22)}100%{transform:scale(1)}}@keyframes pvp-impact-in{from{opacity:0;transform:scale(.7)}to{opacity:1;transform:scale(1)}}\n  @keyframes pvp-wait-pulse{0%,100%{opacity:.35;box-shadow:0 0 0 0 #c084fc55}50%{opacity:1;box-shadow:0 0 0 .45rem #c084fc00}} .pvp-wait-dot{animation:pvp-wait-pulse 1.1s ease-in-out infinite}\n  @media (max-width:640px){ .arena-lobby{gap:1rem}.arena-card-grid{gap:.55rem}.arena-start{bottom:.5rem} }
`}</style></div>;
};
