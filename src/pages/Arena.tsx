...
const deckCompositionIssue = (deck: ArenaCard[]) => {
  if (deck.length !== MAX_DECK_SIZE) return 'Escolha 4 cartas.';
  if (new Set(deck.map((card) => card.id)).size !== MAX_DECK_SIZE) return 'O deck precisa ter 4 cartas diferentes.';
  if (deck.filter((card) => ELITE_RARITIES.has(card.rarity)).length > 2) return 'Use no máximo 2 cartas Lendárias/Míticas no deck.';
  return null;
};
...
export const Arena: React.FC<ArenaProps> = ({ onNavigate }) => {
  ...
  const deckStorageKey = currentUser?.id ? `nexa:arena-deck:${currentUser.id}` : null;
  const presetsStorageKey = currentUser?.id ? `nexa:arena-presets:${currentUser.id}` : null;
  ...
  const loadPreset = (slot: number) => {
    ...
    try {
      ...
      const ids = presets[String(slot)];
      if (!Array.isArray(ids)) { setDeck([]); setActivePreset(slot); return; }
    ...
    } catch { setDeck([]); setActivePreset(slot); }
  };
  const savePreset = (slot: number) => {
    if (!presetsStorageKey || deckCompositionIssue(deck) !== null) return;
    try {
      const presets = JSON.parse(localStorage.getItem(presetsStorageKey) || '{}');
      presets[String(slot)] = deck.map((card) => card.id);
  ...
  useEffect(() => {
    ...
    const validDeck = selectedDeck.filter(isAvailable);
    if (validDeck.length === selectedDeck.length && deckCompositionIssue(validDeck) === null) {
      localStorage.setItem(deckStorageKey, JSON.stringify(validDeck.map((card) => card.id)));
    } else if (selectedDeck.length > 0 && validDeck.length !== selectedDeck.length) {
      setDeck(validDeck);
    }
  ...
  const toggleCard = (card: ArenaCard) => {
    ...
    setDeck((previous) => {
      ...
      if (current.some((item) => item.id === card.id)) return current.filter((item) => item.id !== card.id);
      if (current.length >= MAX_DECK_SIZE) return current;
      if (ELITE_RARITIES.has(card.rarity) && current.filter((item) => ELITE_RARITIES.has(item.rarity)).length >= 2) return current;
      return [...current, card];
    });
  };
