...
export const Arena: React.FC<ArenaProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    if (!pvpRoomId) return;
    let active = true;
    const refresh = async () => { const { data } = await supabase.rpc('get_duelo_nexal_pvp_room', { p_room_id: pvpRoomId }); if (!active || !data) return; setPvpRoomState(data); if (data.status === 'READY' || data.status === 'PLAYING') { if (pvpWaitingMove && Number(data.round) > pvpLastResolvedRound) { setPvpWaitingMove(false); setPvpCardId(null); setPvpNexos(0); setPvpStatus('Rodada resolvida pelo adversário. Próxima jogada liberada.'); } else if (!pvpWaitingMove) setPvpStatus('Adversário conectado. Sala pronta para o duelo.'); } if (data.status === 'FINISHED') { setPvpWaitingMove(false); setPvpStatus('Partida finalizada.'); } };
