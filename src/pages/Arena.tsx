...
export const Arena: React.FC<ArenaProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    if (!pvpRoomId) return;
    let active = true;
    const refresh = async () => { const { data } = await supabase.rpc('get_duelo_nexal_pvp_room', { p_room_id: pvpRoomId }); if (!active || !data) return; setPvpRoomState(data); if (data.status === 'READY' || data.status === 'PLAYING') { if (pvpWaitingMove && Number(data.round) > pvpLastResolvedRound) { const resolvedRound = Number(data.round) - 1; const { data: result } = await supabase.rpc('get_duelo_nexal_pvp_round_result', { p_room_id: pvpRoomId, p_round: resolvedRound }); if (result?.resolved) { setPvpRoundResult(result); setPvpLastResolvedRound(resolvedRound); setPvpReveal(true); window.setTimeout(() => { setPvpReveal(false); setPvpCardId(null); setPvpNexos(0); }, 4200); } setPvpWaitingMove(false); setPvpStatus('Rodada resolvida.'); } else if (!pvpWaitingMove) setPvpStatus('Adversário conectado. Sala pronta para o duelo.'); } if (data.status === 'FINISHED') { if (pvpWaitingMove) { const finalRound = Number(data.round); const { data: result } = await supabase.rpc('get_duelo_nexal_pvp_round_result', { p_room_id: pvpRoomId, p_round: finalRound }); if (result?.resolved && finalRound > pvpLastResolvedRound) { setPvpRoundResult(result); setPvpLastResolvedRound(finalRound); setPvpReveal(true); window.setTimeout(() => setPvpReveal(false), 4200); } } setPvpWaitingMove(false); setPvpStatus('Partida finalizada.'); } };
  ...
  .pvp-reveal-stage{animation:pvp-stage-in .35s ease-out both}.pvp-reveal-card{position:relative;max-width:11rem;margin:auto;padding:.45rem;border:1px solid #ffffff18;border-radius:.7rem;background:#ffffff08;animation:pvp-card-enter .55s cubic-bezier(.2,.8,.2,1) both}.pvp-host{transform-origin:right center}.pvp-guest{transform-origin:left center;animation-name:pvp-card-enter-right}.pvp-combat-stat{margin-top:.35rem;display:grid;grid-template-columns:1fr auto;align-items:center;gap:.1rem .4rem;padding:.35rem .45rem;border-radius:.5rem;background:#020617cc;border:1px solid #ffffff12}.pvp-combat-stat span{font-size:.48rem;font-weight:900;letter-spacing:.12em;color:#94a3b8}.pvp-combat-stat b{grid-row:1/3;grid-column:2;font-size:1.25rem;color:#fde68a;text-shadow:0 0 .7rem #f59e0b88;animation:pvp-attack-charge .8s .55s ease-out both}.pvp-combat-stat small{font-size:.48rem;color:#67e8f9}.pvp-reveal-card:has(.ring-cyan-300){animation-duration:.55s;box-shadow:0 0 1.5rem #22d3ee55}.pvp-vs{animation:pvp-vs-pop .7s .35s ease-out both}.pvp-impact{animation:pvp-impact-in .45s 1.05s ease-out both}@keyframes pvp-stage-in{from{opacity:0}to{opacity:1}}@keyframes pvp-card-enter{from{opacity:0;transform:translateX(-45%) scale(.8)}to{opacity:1;transform:none}}@keyframes pvp-card-enter-right{from{opacity:0;transform:translateX(45%) scale(.8)}to{opacity:1;transform:none}}@keyframes pvp-vs-pop{0%{opacity:0;transform:scale(.2)}65%{opacity:1;transform:scale(1.3)}100%{transform:scale(1)}}@keyframes pvp-attack-charge{0%{opacity:.25;transform:scale(.6)}65%{opacity:1;transform:scale(1.22)}100%{transform:scale(1)}}@keyframes pvp-impact-in{from{opacity:0;transform:scale(.7)}to{opacity:1;transform:scale(1)}}\n  @keyframes pvp-wait-pulse{0%,100%{opacity:.35;box-shadow:0 0 0 0 #c084fc55}50%{opacity:1;box-shadow:0 0 0 .45rem #c084fc00}} .pvp-wait-dot{animation:pvp-wait-pulse 1.1s ease-in-out infinite}\n  @media (max-width:640px){ .arena-lobby{gap:1rem}.arena-card-grid{gap:.55rem}.arena-start{bottom:.5rem} }
`}</style></div>;
};
