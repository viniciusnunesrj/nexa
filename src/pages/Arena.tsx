...
export const Arena: React.FC<ArenaProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    setPvpRoomState(data); const { data: moveState } = await supabase.rpc('get_duelo_nexal_pvp_move_state', { p_room_id: pvpRoomId }); if (moveState?.submitted && !pvpWaitingMove) setPvpWaitingMove(true); if (data.status === 'READY' || data.status === 'PLAYING') { if (pvpWaitingMove && Number(data.round) > pvpLastResolvedRound) { const resolvedRound = Number(data.round) - 1; const { data: result } = await supabase.rpc('get_duelo_nexal_pvp_round_result', { p_room_id: pvpRoomId, p_round: resolvedRound }); if (result?.resolved) { setPvpOpponentUsedCount((count) => Math.max(count, resolvedRound)); setPvpRoundResult(result); setPvpLastResolvedRound(resolvedRound); setPvpReveal(true); setPvpRevealStep(1); window.setTimeout(() => setPvpRevealStep(2), 700); window.setTimeout(() => setPvpRevealStep(3), 1550); window.setTimeout(() => setPvpRevealStep(4), 2450); window.setTimeout(() => { setPvpReveal(false); setPvpRevealStep(0); setPvpCardId(null); setPvpNexos(0); }, 4200); } setPvpWaitingMove(false); setPvpStatus('Rodada resolvida.'); } else if (!pvpWaitingMove) setPvpStatus('Adversário conectado. Sala pronta para o duelo.'); } if (data.status === 'FINISHED') { if (pvpWaitingMove) { const finalRound = Number(data.round); const { data: result } = await supabase.rpc('get_duelo_nexal_pvp_round_result', { p_room_id: pvpRoomId, p_round: finalRound }); if (result?.resolved && finalRound > pvpLastResolvedRound) { setPvpRoundResult(result); setPvpLastResolvedRound(finalRound); setPvpReveal(true); setPvpRevealStep(1); window.setTimeout(() => setPvpRevealStep(2), 700); window.setTimeout(() => setPvpRevealStep(3), 1550); window.setTimeout(() => setPvpRevealStep(4), 2450); window.setTimeout(() => { setPvpReveal(false); setPvpRevealStep(0); }, 4200); } } setPvpWaitingMove(false); setPvpStatus('Partida finalizada.'); } };
