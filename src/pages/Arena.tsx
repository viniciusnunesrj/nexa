...
const DuelView: React.FC<{ duel: DuelState; setDuel: React.Dispatch<React.SetStateAction<DuelState | null>>; onBack: () => void; onGames: () => void }> = ({ duel, setDuel, onBack, onGames }) => {
  ...
  useEffect(() => {
    if (duel.phase === 'REVEAL') {
      const timer = window.setTimeout(() => setDuel((current) => current ? { ...current, phase: 'CHARGE' } : current), 450);
  ...
  return <div className="mx-auto max-w-4xl space-y-4">
    <div className="flex items-center justify-between rounded-xl border border-purple-500/25 bg-[#100d19]/90 p-3 sm:p-4"><div className="min-w-0"><p className="text-xs font-bold uppercase text-red-300">CPU</p><LifeBar hp={duel.cpuHp} /><NexoBar count={duel.cpuNexos} /><p className="mt-1 text-[10px] text-slate-500">Reserva: {duel.cpuCards.map((card) => duel.usedCpu.includes(card.id) ? '✓' : '?').join(' ')}</p></div><p className="px-2 text-center text-xs font-bold uppercase tracking-widest text-purple-300">Rodada {duel.round}/{MAX_ROUNDS}</p><div className="min-w-0 text-right"><p className="text-xs font-bold uppercase text-purple-300">JOGADOR</p><LifeBar hp={duel.playerHp} /><NexoBar count={duel.playerNexos} /></div></div>
    <div className="rounded-[28px] border border-purple-500/20 bg-gradient-to-b from-[#171025] via-[#0e1020] to-[#11121c] px-3 py-5 shadow-[inset_0_0_50px_rgba(124,58,237,0.08)] sm:px-10">
      <div className="flex justify-center"><div className={duel.revealedCpu ? 'animate-[fadeIn_0.4s_ease-out]' : ''}>{duel.revealedCpu ? <BattleCard card={duel.revealedCpu} battle /> : <HiddenCard />}</div></div>
      ...
      <div className="flex items-center justify-center gap-3 py-2 text-purple-300"><span className="text-lg">⚔</span><span className="font-heading text-xl font-black">VS</span><span className="text-lg">⚔</span></div>
      ...
      <div className="flex justify-center"><div className={duel.phase === 'RESULT' ? 'scale-105 transition-transform' : ''}>{selectedCard ? <BattleCard card={selectedCard} battle selected={duel.phase === 'SELECT'} /> : <div className="flex h-[240px] w-[170px] items-center justify-center rounded-2xl border border-dashed border-purple-400/40 text-center text-xs text-slate-500">ESCOLHA UMA<br />CARTA DO DECK</div>}</div></div>
    ...
    <div><p className="mb-2 text-xs font-bold uppercase text-slate-400">Seu Deck · escolha uma carta</p><div className="flex gap-2 overflow-x-auto pb-2 sm:grid sm:grid-cols-6">{duel.playerCards.map((card) => { const used = duel.usedPlayer.includes(card.id); return <button key={card.id} type="button" disabled={used || duel.phase !== 'SELECT'} onClick={() => chooseCard(card.id)} className={`min-w-[130px] text-left transition ${used ? 'opacity-35 grayscale' : ''} ${duel.selectedId === card.id ? 'ring-2 ring-purple-400' : ''}`}><BattleCard card={card} compact selected={duel.selectedId === card.id} /><span className="block -mt-5 px-3 pb-2 text-[9px] font-bold uppercase text-slate-500">{used ? 'USADA' : 'DISPONÍVEL'}</span></button>; })}</div></div>
  </div>;
};
...
const HiddenCard: React.FC = () => <div className="flex h-[240px] w-[170px] flex-col items-center justify-center rounded-2xl border-2 border-red-400/30 bg-[#171323] text-center text-red-200 shadow-[0_0_25px_rgba(239,68,68,0.12)]"><span className="font-heading text-2xl font-black">NEXA</span><span className="mt-2 text-[10px] uppercase">Carta oculta</span></div>;
