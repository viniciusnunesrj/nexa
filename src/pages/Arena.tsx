...
export const Arena: React.FC<ArenaProps> = ({ onNavigate }) => {
  ...
  const pvpResolvedRound = Math.max(0, Number(pvpRoomState?.round || 1) - 1);
  const pvpResolvedCount = pvpRoomState?.status === 'FINISHED' ? Number(pvpRoomState?.round || 0) : pvpResolvedRound;
  useEffect(() => {
  ...
  const pvpServerRound = Number(pvpRoomState?.round || 1);
  const pvpRoundHistory = pvpHistory.filter((item) => Number(item.round) <= pvpResolvedCount);
