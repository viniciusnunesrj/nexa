...
const PvpNexosControls: React.FC<{
  value: number; max: number; disabled: boolean; submitting: boolean; canConfirm: boolean;
  onChange: (value: number) => void; onConfirm: () => void;
}> = ({ value, max, disabled, submitting, canConfirm, onChange, onConfirm }) => (
  <div className="mt-3 rounded-xl border border-cyan-400/10 bg-black/20 p-3">
    <div className="mb-2 flex items-center justify-between">
      <span className="text-[8px] font-black uppercase tracking-[.16em] text-slate-400">Investimento secreto</span>
      <strong className="text-sm text-cyan-200">{value} <span className="text-[9px]">NEXOS</span></strong>
    </div>
    <div className="flex items-center gap-3">
      <button type="button" disabled={disabled || value <= 0} onClick={() => onChange(Math.max(0, value - 1))} className="h-8 w-8 rounded-full border border-white/10 text-sm font-black text-slate-200 disabled:opacity-25">−</button>
      <input type="range" min="0" max={max} value={value} disabled={disabled} onChange={(e) => onChange(Number(e.target.value))} className="flex-1" />
      <button type="button" disabled={disabled || value >= max} onClick={() => onChange(Math.min(max, value + 1))} className="h-8 w-8 rounded-full border border-white/10 text-sm font-black text-slate-200 disabled:opacity-25">+</button>
      <button type="button" disabled={!canConfirm || disabled} onClick={onConfirm} className="rounded-lg bg-cyan-500 px-4 py-2 text-[10px] font-black uppercase text-[#04131b] disabled:opacity-40">{submitting ? 'Enviando...' : 'Confirmar'}</button>
    </div>
  </div>
);
