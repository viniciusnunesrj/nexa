...
const DuelView: React.FC<{ duel: DuelState; setDuel: React.Dispatch<React.SetStateAction<DuelState | null>>; onBack: () => void; onGames: () => void }> = ({ duel, setDuel, onBack, onGames }) => {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="relative z-10 mb-1 flex justify-center gap-1"><span className="mr-1 self-center text-[8px] font-bold uppercase tracking-widest text-slate-500">CPU</span>{duel.cpuCards.map((card) => <div key={card.id} className={`flex h-5 w-5 items-center justify-center rounded border bg-[#171323] text-[9px] ${duel.usedCpu.includes(card.id) ? 'border-red-400/50 text-red-300' : 'border-purple-300/30 text-purple-200/70'}`}>{duel.usedCpu.includes(card.id) ? '✓' : '◇'}</div>)}</div>
    <div className="relative overflow-hidden rounded-[28px] border border-purple-400/25 bg-[radial-gradient(circle_at_center,rgba(99,52,160,0.2),transparent_45%),linear-gradient(180deg,#120d20,#080b16)] px-3 py-3 shadow-[0_20px_90px_rgba(35,14,73,0.35)] sm:px-8 sm:py-4">
      ...
      <div className="my-1 text-center text-[10px] font-bold uppercase tracking-widest text-amber-300">{showNumbers ? `${duel.cpuInvestment} NEXOS · ${duel.cpuAttack} ATK` : 'NEXOS OCULTOS'}</div>
      <div className={`flex items-center justify-center gap-2 py-1 text-purple-300 ${duel.phase === 'CHARGE' ? 'scale-110 transition-transform' : ''}`}><span>⚔</span><span className="font-heading text-lg font-black">VS</span><span>⚔</span></div>
      <div className="my-1 text-center text-[10px] font-bold uppercase tracking-widest text-amber-300">{selectedCard && duel.phase === 'SELECT' ? `${duel.investment} NEXOS · ${attackValue(selectedCard, duel.investment)} ATK` : showNumbers ? `${duel.investment} NEXOS · ${duel.playerAttack} ATK` : 'ESCOLHA UMA CARTA'}</div>
      <div className="flex justify-center"><div className={duel.phase === 'RESULT' ? 'scale-105 transition-transform' : duel.phase === 'CHARGE' ? 'translate-y-1 transition-transform' : ''}>{selectedCard ? <BattleCard card={selectedCard} battle selected={duel.phase === 'SELECT'} /> : <div className="flex h-[220px] w-[165px] items-center justify-center rounded-2xl border border-dashed border-purple-400/40 text-center text-xs text-slate-500">ESCOLHA UMA<br />CARTA</div>}</div></div>
    ...
    <div className="relative mt-2"><p className="mb-1 text-center text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400">Seu Deck · escolha uma carta</p><div className="flex gap-1.5 overflow-x-auto pb-1 sm:justify-center">{duel.playerCards.map((card) => { const used = duel.usedPlayer.includes(card.id); return <button key={card.id} type="button" disabled={used || duel.phase !== 'SELECT'} onClick={() => chooseCard(card.id)} className={`min-w-[92px] text-left transition ${used ? 'opacity-30 grayscale' : ''} ${duel.selectedId === card.id ? '-translate-y-1 ring-2 ring-purple-400' : ''}`}><BattleCard card={card} compact selected={duel.selectedId === card.id} /><span className="block px-2 text-[8px] font-bold uppercase text-slate-500">{used ? '✓ USADA' : 'DISPONÍVEL'}</span></button>; })}</div></div>
  </div>;
};
