...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid items-stretch gap-3 lg:grid-cols-[1.05fr_1fr_1.15fr]">
        <button onClick={() => onNavigate('progression')} className="group min-h-[150px] rounded-[14px] border border-cyan-400/35 bg-[#06111b] p-3.5 text-left shadow-[0_0_26px_rgba(34,211,238,.06),0_16px_40px_rgba(0,0,0,.3)] transition hover:border-cyan-300/60">
          ...
          <div className="mt-2.5 flex items-center gap-2"><Award className="h-4 w-4 text-sky-400"/><p className="text-[11px] font-bold text-slate-100">{isMaxLevel?'Progressão concluída':`Alcance o nível ${nextLevel}`}</p></div>
          ...
          <div className="mt-2.5 flex items-center justify-between border-t border-cyan-400/10 pt-2 text-[8px] font-bold uppercase tracking-wider text-cyan-500"><span>Progressão do piloto</span><ChevronRight className="h-3.5 w-3.5 text-cyan-300 transition group-hover:translate-x-1"/></div>
        </button>

        <button onClick={() => onNavigate('collections')} className="group min-h-[150px] overflow-hidden rounded-[14px] border border-fuchsia-400/35 bg-[#110817] p-3.5 text-left shadow-[0_0_28px_rgba(217,70,239,.07),0_16px_40px_rgba(0,0,0,.3)] transition hover:border-fuchsia-300/60">
          ...
          <div className="relative mt-3 flex h-[72px] items-center gap-2 overflow-hidden">
            ...
            {userItems.slice(0,4).map((item,i)=><div key={item.id} className={`relative h-[58px] min-w-[46px] overflow-hidden rounded-lg border bg-black/40 shadow-[0_0_16px_rgba(217,70,239,.10)] ${i===1?'border-fuchsia-300/55 -translate-y-1':'border-white/10'}`}><CardImage asset={item} src={item.image} alt={item.name} className="h-full w-full object-cover"/><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent px-1 pb-1 pt-4 text-center text-[6px] font-bold text-white">{item.name}</div></div>)}
          ...
          <div className="mt-1.5 grid grid-cols-2 gap-2 text-[8px]"><div className="flex items-center gap-2 rounded-lg border border-violet-400/10 bg-violet-400/[.04] px-2 py-1.5"><Trophy className="h-3.5 w-3.5 text-violet-300"/><span className="text-slate-400"><b className="text-violet-200">{distinctGuardiansCards}/4</b> Guardiões</span></div><div className="flex items-center gap-2 rounded-lg border border-amber-400/10 bg-amber-400/[.04] px-2 py-1.5"><PackageOpen className="h-3.5 w-3.5 text-amber-300"/><span className="text-slate-400"><b className="text-amber-200">{myBoxes.length}</b> Caixas</span></div></div>
        </button>

        <div className="min-h-[150px] rounded-[14px] border border-sky-400/30 bg-[#070e17] p-3.5 shadow-[0_0_26px_rgba(56,189,248,.06),0_16px_40px_rgba(0,0,0,.3)]">
