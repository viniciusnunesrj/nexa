  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:gap-4 lg:grid-cols-2">
        <button onClick={() => onNavigate('riftbattle-v2')} className="group relative min-h-[190px] overflow-hidden rounded-[18px] border border-cyan-400/45 bg-[#06121c] p-5 text-left shadow-[0_22px_55px_rgba(0,0,0,.28),inset_0_1px_0_rgba(255,255,255,.035)] transition duration-300 hover:-translate-y-1 hover:border-cyan-300/80 hover:shadow-[0_28px_65px_rgba(0,0,0,.38),0_0_35px_rgba(34,211,238,.10)]">
          ...
          <div className="relative flex h-full max-w-full flex-col justify-between pr-20 sm:max-w-[72%] sm:pr-0">
            <div><div className="flex items-center gap-2"><span className="font-mono text-[9px] font-black uppercase tracking-[.18em] text-cyan-300">Jogo principal</span><span className="rounded-full border border-emerald-400/25 bg-emerald-400/[.07] px-2 py-0.5 text-[8px] font-bold uppercase text-emerald-300">● Online</span></div><h2 className="mt-1.5 font-heading text-[22px] font-black leading-none text-white">RIFT BATTLE <span className="text-fuchsia-400 drop-shadow-[0_0_10px_rgba(232,121,249,.45)]">V2</span></h2><p className="mt-2 text-[11px] leading-[1.45] text-slate-400">Combate tático em equipe. Forme seu esquadrão, enfrente o Rift e conquiste recompensas.</p><div className="mt-3 flex flex-wrap gap-1.5">{['PvE','2×2','3×3','4×4','NEX + NXA + XP'].map(x=><span key={x} className="rounded-full border border-cyan-400/20 bg-black/20 px-2.5 py-1 text-[8px] font-bold text-slate-300">{x}</span>)}</div></div>
      ...
      <div className="grid gap-3 lg:grid-cols-[1.05fr_1fr_1.15fr]">
        <button onClick={() => onNavigate('progression')} className="group rounded-[16px] border border-cyan-400/35 bg-[#06111b] p-4 text-left shadow-[0_0_26px_rgba(34,211,238,.06),0_16px_40px_rgba(0,0,0,.3)] transition hover:border-cyan-300/60">
          <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg border border-cyan-300/25 bg-cyan-400/[.08] shadow-[0_0_14px_rgba(34,211,238,.14)]"><TrendingUp className="h-4 w-4 text-cyan-300"/></span><h3 className="font-heading text-[11px] font-black uppercase tracking-[.13em] text-cyan-300 drop-shadow-[0_0_8px_rgba(34,211,238,.25)]">Próximo objetivo</h3></div><span className="rounded-md border border-cyan-400/25 bg-cyan-400/[.06] px-2 py-1 text-[8px] font-mono font-bold text-cyan-300">NV. {isMaxLevel?MAX_GAME_LEVEL:nextLevel}</span></div>
          <div className="mt-3 flex items-center gap-2"><Award className="h-4 w-4 text-sky-400"/><p className="text-[11px] font-bold text-slate-100">{isMaxLevel?'Progressão concluída':`Alcance o nível ${nextLevel}`}</p></div>
          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/[.07]"><div style={{width:`${xpPercent}%`}} className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-sky-500 shadow-[0_0_10px_rgba(34,211,238,.65)]"/></div>
          <div className="mt-2 flex justify-between text-[8px] font-mono text-slate-400"><span className="flex items-center gap-1"><Zap className="h-3 w-3 text-cyan-400"/>{currentExp}/{maxExp} XP</span><span>{isMaxLevel?'MAX':`Faltam ${Math.max(0,maxExp-currentExp)} XP`}</span></div>
          <div className="mt-3 flex items-center justify-between border-t border-cyan-400/10 pt-2 text-[8px] font-bold uppercase tracking-wider text-cyan-500"><span>Progressão do piloto</span><ChevronRight className="h-3.5 w-3.5 text-cyan-300 transition group-hover:translate-x-1"/></div>
        </button>

        <button onClick={() => onNavigate('collections')} className="group overflow-hidden rounded-[16px] border border-fuchsia-400/35 bg-[#110817] p-4 text-left shadow-[0_0_28px_rgba(217,70,239,.07),0_16px_40px_rgba(0,0,0,.3)] transition hover:border-fuchsia-300/60">
          <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg border border-fuchsia-300/25 bg-fuchsia-400/[.09] shadow-[0_0_14px_rgba(217,70,239,.18)]"><Gift className="h-4 w-4 text-fuchsia-300"/></span><h3 className="font-heading text-[11px] font-black uppercase tracking-[.13em] text-fuchsia-300 drop-shadow-[0_0_8px_rgba(217,70,239,.3)]">Coleção & Caixas</h3></div><span className="text-[8px] font-mono text-fuchsia-400">Ver todas →</span></div>
          <div className="relative mt-3 flex h-[72px] items-center gap-2 overflow-hidden">
            <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-5 bg-gradient-to-r from-[#110817] to-transparent"/><div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-5 bg-gradient-to-l from-[#110817] to-transparent"/>
            {userItems.slice(0,4).map((item,i)=><div key={item.id} className={`relative h-[68px] min-w-[52px] overflow-hidden rounded-lg border bg-black/40 shadow-[0_0_16px_rgba(217,70,239,.10)] ${i===1?'border-fuchsia-300/55 -translate-y-1':'border-white/10'}`}><CardImage asset={item} src={item.image} alt={item.name} className="h-full w-full object-cover"/><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent px-1 pb-1 pt-4 text-center text-[6px] font-bold text-white">{item.name}</div></div>)}
            {userItems.length===0&&<div className="flex h-full w-full items-center justify-center rounded-lg border border-dashed border-fuchsia-400/20 text-[8px] text-slate-500">Sua coleção aparecerá aqui</div>}
          </div>
          <div className="mt-2 grid grid-cols-2 gap-2 text-[8px]"><div className="flex items-center gap-2 rounded-lg border border-violet-400/10 bg-violet-400/[.04] px-2 py-1.5"><Trophy className="h-3.5 w-3.5 text-violet-300"/><span className="text-slate-400"><b className="text-violet-200">{distinctGuardiansCards}/4</b> Guardiões</span></div><div className="flex items-center gap-2 rounded-lg border border-amber-400/10 bg-amber-400/[.04] px-2 py-1.5"><PackageOpen className="h-3.5 w-3.5 text-amber-300"/><span className="text-slate-400"><b className="text-amber-200">{myBoxes.length}</b> Caixas</span></div></div>
        </button>

        <div className="rounded-[16px] border border-sky-400/30 bg-[#070e17] p-4 shadow-[0_0_26px_rgba(56,189,248,.06),0_16px_40px_rgba(0,0,0,.3)]">
          <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg border border-sky-300/25 bg-sky-400/[.08]"><Clock className="h-4 w-4 text-sky-300"/></span><h3 className="font-heading text-[11px] font-black uppercase tracking-[.13em] text-sky-300 drop-shadow-[0_0_8px_rgba(56,189,248,.25)]">Atividade recente</h3></div><button onClick={()=>onNavigate('history')} className="text-[8px] text-slate-500 hover:text-sky-300">Ver todas →</button></div>
          <div className="mt-3 space-y-1.5">
            {transactions.slice(0,4).map((tx,i)=><div key={tx.id} className="flex items-center justify-between gap-2 border-b border-white/[.05] pb-1.5 text-[8px] last:border-0"><div className="flex min-w-0 items-center gap-2"><span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md border ${i%3===0?'border-cyan-400/20 bg-cyan-400/[.07] text-cyan-300':i%3===1?'border-fuchsia-400/20 bg-fuchsia-400/[.07] text-fuchsia-300':'border-amber-400/20 bg-amber-400/[.07] text-amber-300'}`}>{i%3===0?<ArrowLeftRight className="h-3 w-3"/>:i%3===1?<Sparkles className="h-3 w-3"/>:<PackageOpen className="h-3 w-3"/>}</span><div className="min-w-0"><p className="truncate font-bold text-slate-200">{tx.itemSnapshot.name}</p><p className="truncate text-slate-600">{tx.buyerName} comprou de {tx.sellerName}</p></div></div><span className="shrink-0 font-bold text-cyan-300">+{formatEconomicValue(tx.amount)} NXA</span></div>)}
