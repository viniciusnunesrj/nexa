      ...
      <section className="relative isolate overflow-hidden rounded-[22px] border border-cyan-400/25 bg-[#040811] shadow-[0_30px_100px_rgba(0,0,0,.52),0_0_45px_rgba(34,211,238,.06)]">
        ...
        <div className="relative min-h-[390px] px-4 py-5 sm:min-h-[345px] sm:px-7 lg:h-[344px] lg:min-h-0 lg:px-8">
          ...
          <div className="absolute inset-x-4 bottom-3 z-20 grid grid-cols-3 overflow-hidden rounded-xl border border-cyan-300/20 bg-[#06101b]/95 backdrop-blur-md sm:inset-x-7 lg:left-[7%] lg:right-[7%] lg:grid-cols-5">
            <div className="flex min-h-[58px] items-center gap-3 border-r border-white/[.07] px-4"><Layers className="h-5 w-5 text-cyan-300"/><div><p className="font-heading text-base font-black text-white">{userCards.length}</p><p className="text-[8px] uppercase tracking-[.10em] text-slate-400">Cartas no inventário</p></div></div>
            <div className="flex min-h-[58px] items-center gap-3 border-r border-white/[.07] px-4"><Award className="h-5 w-5 text-sky-300"/><div><p className="font-heading text-base font-black text-cyan-300">Nv. {currentLevel}</p><p className="text-[8px] uppercase tracking-[.10em] text-slate-400">Piloto</p></div></div>
      ...
      <div className="grid gap-3 sm:gap-4 lg:grid-cols-2">
        <button onClick={() => onNavigate('riftbattle-v2')} className="group relative min-h-[178px] overflow-hidden rounded-[18px] border border-cyan-400/45 bg-[#06121c] p-4 text-left shadow-[0_22px_55px_rgba(0,0,0,.28),inset_0_1px_0_rgba(255,255,255,.035)] transition duration-300 hover:-translate-y-1 hover:border-cyan-300/80 hover:shadow-[0_28px_65px_rgba(0,0,0,.38),0_0_35px_rgba(34,211,238,.10)]">
          ...
          <div className="relative z-10 flex h-full max-w-full flex-col justify-between pr-20 sm:max-w-[68%] sm:pr-0">
            <div><div className="flex items-center gap-2"><span className="font-mono text-[10px] font-black uppercase tracking-[.16em] text-cyan-300">Jogo principal</span><span className="rounded-full border border-emerald-400/25 bg-emerald-400/[.07] px-2 py-0.5 text-[9px] font-bold uppercase text-emerald-300">● Online</span></div><h2 className="mt-1.5 font-heading text-[20px] font-black leading-none text-white">RIFT BATTLE <span className="text-fuchsia-400 drop-shadow-[0_0_10px_rgba(232,121,249,.45)]">V2</span></h2><p className="mt-1.5 text-[11px] leading-[1.4] text-slate-300">Combate tático em equipe. Forme seu esquadrão, enfrente o Rift e conquiste recompensas.</p><div className="mt-2.5 flex flex-wrap gap-1.5">{['PvE','2×2','3×3','4×4','NEX + NXA + XP'].map(x=><span key={x} className="rounded-full border border-cyan-400/20 bg-black/20 px-2.5 py-1 text-[9px] font-bold text-slate-200">{x}</span>)}</div></div>
        ...
        <button onClick={() => onNavigate('arena')} className="group relative min-h-[178px] overflow-hidden rounded-[18px] border border-fuchsia-400/45 bg-[#110918] p-4 text-left shadow-[0_22px_55px_rgba(0,0,0,.28),inset_0_1px_0_rgba(255,255,255,.035)] transition duration-300 hover:-translate-y-1 hover:border-fuchsia-300/80 hover:shadow-[0_28px_65px_rgba(0,0,0,.38),0_0_35px_rgba(217,70,239,.10)]">
          ...
          <div className="relative z-10 flex h-full max-w-full flex-col justify-between pr-20 sm:max-w-[68%] sm:pr-0">
            <div><div className="flex items-center gap-2"><span className="font-mono text-[10px] font-black uppercase tracking-[.16em] text-fuchsia-300">Duelo estratégico</span><span className="rounded-full border border-emerald-400/25 bg-emerald-400/[.07] px-2 py-0.5 text-[9px] font-bold uppercase text-emerald-300">● Online</span></div><h2 className="mt-1.5 font-heading text-[20px] font-black leading-none text-white">NEXUS DUEL</h2><p className="mt-1.5 text-[11px] leading-[1.4] text-slate-300">Duelo individual 4×4. Use suas cartas, Nexos e habilidades em confrontos PvE ou PvP.</p><div className="mt-2.5 flex flex-wrap gap-1.5">{['PvE','PvP','Deck 4×4','Habilidades','XP compartilhado'].map(x=><span key={x} className="rounded-full border border-fuchsia-400/20 bg-black/20 px-2.5 py-1 text-[9px] font-bold text-slate-200">{x}</span>)}</div></div>
      ...
      <div className="grid items-stretch gap-3 lg:grid-cols-[1.02fr_1.08fr_.98fr]">
        <button onClick={()=>onNavigate('progression')} className="group h-[172px] rounded-[14px] border border-cyan-400/30 bg-[#06111b] p-3.5 text-left shadow-[0_0_24px_rgba(34,211,238,.06)] transition hover:border-cyan-300/50">
          ...
          <div className="mt-2 space-y-1.5">
            <div className="grid grid-cols-[22px_1fr_auto] items-center gap-2"><span className="flex h-5 w-5 items-center justify-center rounded border border-cyan-400/20 bg-cyan-400/[.07]"><Award className="h-3 w-3 text-cyan-300"/></span><span className="truncate text-[10px] font-bold text-slate-200">{isMaxLevel?'Progressão concluída':`Alcance o nível ${nextLevel}`}</span><span className="text-[7px] font-bold text-cyan-300">{currentLevel}/{isMaxLevel?MAX_GAME_LEVEL:nextLevel}</span></div>
        ...
        <div className="h-[172px] rounded-[14px] border border-sky-400/25 bg-[#070e17] p-3.5 shadow-[0_0_24px_rgba(56,189,248,.06)]">
          ...
          <div className="mt-1.5 space-y-1">
            {transactions.slice(0,3).map((tx,i)=><div key={tx.id} className="grid grid-cols-[22px_1fr_auto] items-center gap-2 border-b border-white/[.05] pb-1"><span className={`flex h-5 w-5 items-center justify-center rounded border ${i===0?'border-cyan-400/20 bg-cyan-400/[.07] text-cyan-300':i===1?'border-fuchsia-400/20 bg-fuchsia-400/[.07] text-fuchsia-300':'border-amber-400/20 bg-amber-400/[.07] text-amber-300'}`}>{i===0?<ArrowLeftRight className="h-3 w-3"/>:i===1?<Sparkles className="h-3 w-3"/>:<PackageOpen className="h-3 w-3"/>}</span><div className="min-w-0"><p className="truncate text-[9px] font-bold text-slate-200">{tx.itemSnapshot.name}</p><p className="truncate text-[7px] text-slate-500">{tx.buyerName} comprou de {tx.sellerName}</p></div><span className="text-[7px] font-bold text-cyan-300">+{formatEconomicValue(tx.amount)} NXA</span></div>)}
