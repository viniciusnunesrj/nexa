...
export const CurrencyBadge: React.FC<CurrencyBadgeProps> = ({
  ...
  return (
    <div
      className={`inline-flex min-w-0 shrink items-center gap-1.5 rounded-lg font-mono font-bold tracking-tight border backdrop-blur-md ${sizeClasses[size]} ${
        isNEX
    ...
    >
      ...
      <span className="min-w-0 truncate">{formatEconomicValue(Number(amount || 0))}</span>
      {showLabel && (
        <span className={`max-[440px]:hidden text-[10px] font-sans opacity-75 uppercase tracking-wider ${isNEX ? 'text-amber-400' : 'text-cyan-400'}`}>
          {type}
        </span>
      )}
    </div>
