...
BEGIN
  ...
  IF p_card_ids IS NULL OR cardinality(p_card_ids) <> 3 THEN
  ...
  IF FOUND THEN
    IF v_existing.input_card_ids <> v_card_ids THEN
      RAISE EXCEPTION 'request_id já utilizado com outras cartas';
    END IF;
   ...
   * - state FREE obrigatório;
   * - template oficial ativo obrigatório.
   *
   * Não exigimos card_status/status porque cartas históricas
   * podem possuir valores legados ou nulos nesses campos.
   */
  SELECT COUNT(*)
  INTO v_valid_cards
  FROM public.user_cards c
  ...
  IF v_valid_cards <> 3 THEN
    RAISE EXCEPTION
      'Carta inexistente, não pertencente à conta ou indisponível para fusão';
  END IF;
  ...
  IF v_rarity_count <> 1 OR v_input_rarity IS NULL THEN
    RAISE EXCEPTION 'As 3 cartas devem possuir a mesma raridade';
  END IF;
  ...
  CASE v_input_rarity
    ...
    WHEN 'Mítico' THEN
      RAISE EXCEPTION 'Cartas Míticas já atingiram a raridade máxima';
  ...
  IF v_profile.balance_nex < v_charge THEN
    RAISE EXCEPTION 'Saldo NEX insuficiente para concluir a fusão';
  END IF;
  ...
  IF v_success THEN
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Nenhum template oficial disponível para a raridade de saída';
    END IF;
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Metadados oficiais da carta resultante não encontrados';
    END IF;
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Falha ao consumir cartas da fusão';
    END IF;

    v_output_card_id := 'card-' || gen_random_uuid()::text;
    ...
    v_result := jsonb_build_object(
      ...
      'balance_nex', v_profile.balance_nex - v_charge,
      'burned_card_ids', to_jsonb(v_burned_card_ids),
      'preserved_card_id', NULL,
      'output_template_id', v_output_template.template_id,
      'output_card', to_jsonb(v_output_card)
    );
  ...
  ELSE
    ...
    v_result := jsonb_build_object(
      ...
      'balance_nex', v_profile.balance_nex - v_charge,
      'burned_card_ids', to_jsonb(v_burned_card_ids),
      'preserved_card_id', v_preserved_card_id,
  ...
  VALUES (
    ...
    v_success,
    v_charge,
    CASE WHEN v_success THEN v_output_card.id ELSE NULL END,
    CASE WHEN v_success THEN v_output_template.template_id ELSE NULL END,
    v_burned_card_ids,
    v_preserved_card_id,
    v_result
  );
